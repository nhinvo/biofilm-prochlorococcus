...
def main():
    Path('data/AbsoluteGE_IndividualEfficiency').mkdir(exist_ok=True)

    # 1. Import table of all extraction efficiencies 
    efficiencies = "data/EfficiencyMetadata/StandardData.tsv"
    efficiencies_df = pd.read_table(efficiencies)[['sample', 'Efficiency']]
    efficiencies_df = efficiencies_df.rename(columns={'sample': 'sample_name'})
    ...
    df = pd.merge(df, efficiencies_df, on='sample_name', how='inner')
    ...
    df.to_csv('data/AbsoluteGE_IndividualEfficiency/SubcladeAbosluteGenomeEquivalent.tsv', sep='\t', index=False)
    df.to_excel('data/AbsoluteGE_IndividualEfficiency/SubcladeAbosluteGenomeEquivalent.xlsx', index=False)
    ...
    df.to_csv('data/AbsoluteGE_IndividualEfficiency/CladeAbosluteGenomeEquivalent.tsv', sep='\t', index=False)  
    df.to_excel('data/AbsoluteGE_IndividualEfficiency/CladeAbosluteGenomeEquivalent.xlsx', index=False)    
